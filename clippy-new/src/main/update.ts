...
export function setupAutoUpdater() {
  ...
  getLogger().info("Auto-updater skipped on Linux");
}
...
export async function checkForUpdates() {
  if (!app.isPackaged) {
    return dialog.showMessageBox({
      ...
      title: "Update Check",
      message:
        "You are running a development version of Clippy, so the update check is disabled.",
    });
  }
  ...
  try {
    ...
    if (
      ...
      if (result.response === 0) {
        shell.openExternal(
          "https://github.com/felixrieseberg/clippy/releases/latest",
        );
      }
    } else {
