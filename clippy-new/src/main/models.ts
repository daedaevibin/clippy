...
class ModelManager {
  ...
  private get models() {
    return (getStateManager().store as any).get("models");
  }

  private set models(models: ModelState) {
    (getStateManager().store as any).set("models", models);
  }
  ...
  public async downloadModelByName(name: string) {
    ...
    if ((getDebugManager().store as any).get("simulateDownload")) {
    ...
    for (const model of Object.values(this.models)) {
      ...
      const downloadState: DownloadState | undefined = downloadItem
        ? {
            ...
            percentComplete:
              "getPercentComplete" in downloadItem
                ? (downloadItem as any).getPercentComplete()
            ...
            currentBytesPerSecond:
              "getCurrentBytesPerSecond" in downloadItem
                ? (downloadItem as any).getCurrentBytesPerSecond()
                : 0,
  ...
  public getIsModelDownloaded(model: ManagedModel | Model): boolean {
    ...
    if ((getDebugManager().store as any).get("simulateNoModelsDownloaded")) {
      return false;
    }
