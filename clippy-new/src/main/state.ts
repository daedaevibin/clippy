...
export class StateManager {
  ...
  constructor() {
    ...
    (this.store as any).onDidAnyChange(this.onDidAnyChange);

    // Handle settings changes
    (this.store as any).onDidChange(
      "settings",
      (newValue: SettingsState, oldValue: SettingsState) => {
  ...
  public updateModelState() {
    (this.store as any).set(
      "models",
      getModelManager().getRendererModelState(),
    );
  }
  ...
  private ensureCorrectSettingsState() {
    ...
    if (settings.selectedModel) {
      const model = (this.store as any).get("models")[settings.selectedModel];
  ...
  private ensureCorrectModelState() {
    const models = (this.store as any).get("models");
    ...
    (this.store as any).set("models", models);
  }
  ...
  public onDidAnyChange(newValue: SharedState = (this.store as any).store) {
