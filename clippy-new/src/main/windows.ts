...
export async function createMainWindow() {
  ...
  const settings = (getStateManager().store as any).get("settings");
    ...
    (_event: Electron.Event, browserWindow: BrowserWindow) => {
      ...
      */
      ...
      browserWindow.webContents.on("did-finish-load", () => {
        setFontSize(
          (getStateManager().store as any).get("settings").defaultFontSize,
          [browserWindow],
        );
        setFont((getStateManager().store as any).get("settings").defaultFont, [
          browserWindow,
        ]);
      });
    },
  ...
  browserWindow.webContents.setWindowOpenHandler(({ url, features }) => {
    ...
    return {
      ...
      overrideBrowserWindowOptions: {
        ...
        minHeight: 400,
        minWidth: 400,
        alwaysOnTop: (getStateManager().store as any).get("settings")
          .chatAlwaysOnTop,
